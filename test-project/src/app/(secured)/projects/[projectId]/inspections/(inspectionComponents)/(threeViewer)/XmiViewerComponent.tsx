"use client";
import { findCenter } from "@/app/utils/CenterFinder";
import { rescale } from "@/app/utils/Rescale";
...
export default function XmiViewerComponent({
  ...
  async function DownloadDataFromS3AndParseToson(path: string) {
  ...
  async function fetchAndCreateObjects(scene: THREE.Scene) {
    try {
      const [data, matchingPairsData] = await Promise.all([
        DownloadDataFromS3AndParseToson(pathGroup[0].filePath),
        DownloadDataFromS3AndParseToson(pathGroup[0].resultPath),
      ]);
